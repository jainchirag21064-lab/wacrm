...
//   RPCs (set_member_role, deactivate_account_member,
//   reactivate_account_member, owner-only delete_account_member)
//   double-check the role anyway.
//
// Member removal semantics (migration 045):
//   - "Remove member" = DEACTIVATE: access revoked instantly,
//     reversible via Restore, but the member keeps occupying a seat.
//   - "Delete permanently" = owner-only, irreversible; the only way
//     to free a seat occupied by a deactivated member.
//   Seat usage (non-owner members, incl. deactivated) is shown
//   against the platform-configured member limit.
// ============================================================
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      const mdata = (await mres.json()) as MembersPayload;
      setMembers(mdata.members);
      setMemberLimit(mdata.memberLimit);
  ...
  async function handleDeactivate() {
    ...
    try {
      ...
      toast.success(
        t('deactivatedToast', { name: member.full_name || t('unnamed') })
      );
      // Deactivation is reversible and seat-preserving: keep the row in
      // the roster (muted) so it can be restored or permanently deleted.
      setMembers((prev) =>
        prev.map((m) =>
      ...
      console.error('[MembersTab] deactivate error:', err);
      toast.error('Could not reach the server');
    } finally {
      setPendingMemberAction(null);
    }
  ...
  async function handleReactivate(member: Member) {
    ...
    try {
      const res = await fetch(
        `/api/account/members/${member.user_id}/reactivate`,
        { method: 'POST' }
      );
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || 'Failed to restore member');
        return;
      }
      toast.success(
        t('reactivatedToast', { name: member.full_name || t('unnamed') })
      );
      setMembers((prev) =>
        prev.map((m) =>
          m.user_id === member.user_id
            ? { ...m, deactivated_at: null, status: 'active' as const }
            : m
        )
      );
    } catch (err) {
      console.error('[MembersTab] reactivate error:', err);
      toast.error('Could not reach the server');
    } finally {
      setPendingMemberAction(null);
    }
  ...
  async function handleDeletePermanent() {
    ...
    setPendingMemberAction(member.user_id);
    try {
      const res = await fetch(`/api/account/members/${member.user_id}/delete`, {
        method: 'POST',
      });
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || 'Failed to delete member');
        return;
      }
      toast.success(
        t('deletedToast', { name: member.full_name || t('unnamed') })
      );
      setMembers((prev) => prev.filter((m) => m.user_id !== member.user_id));
      setDeletingMember(null);
    } catch (err) {
      console.error('[MembersTab] delete error:', err);
      toast.error('Could not reach the server');
    } finally {
      setPendingMemberAction(null);
    }
  ...
  // Seat accounting: non-owner members, including deactivated ones
  // (a deactivated member keeps occupying the seat until an owner
  // permanently deletes them).
  const seatUsage = members.filter((m) => m.role !== 'owner').length;
  const seatFull = seatUsage >= memberLimit;
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
        ...
        action={
          <RequireRole min="admin">
            <Button
              ...
              disabled={seatFull}
              title={seatFull ? t('seatsFull') : undefined}
            >
      ...
      {/* Live presence summary across the active roster. Deactivated
          members drop out of counts (their presence rows are gone).
          Updates without a full refresh as heartbeats and the local
          re-derive tick land. */}
      {members.length > 0 &&
        (() => {
          const activeMembers = members.filter((m) => m.status === 'active');
          const deactivatedCount = members.length - activeMembers.length;
          const counts = summarize(
            activeMembers.map((m) => getPresence(m.user_id))
          );
          return (
            <div className="text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
              ...
              {deactivatedCount > 0 && (
                <span className="text-muted-foreground/70">
                  · {t('deactivatedCount', { count: deactivatedCount })}
                </span>
              )}
            </div>
          );
      ...
      {/* Seat usage against the platform-configured member limit. */}
      {members.length > 0 && (
        <div
          className={`flex flex-wrap items-center gap-x-2 gap-y-1 rounded-md border px-3 py-2 text-xs ${
            seatFull
        ...
        >
          ...
          <span className="font-medium">
            {t('seatsLabel', { used: seatUsage, limit: memberLimit })}
          </span>
          {seatFull && <span>{t('seatsFull')}</span>}
        </div>
      )}
            ...
            {members.map((member) => {
              ...
              return (
                <li
                  ...
                  // before. Deactivated rows are muted to read as
                ...
                >
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    {isDeactivated ? (
                      <>
                        {/* Restore — admin+; gives the member back
                            their seat + access. */}
                        {canManageMembers && (
                          <Button
                            variant="outline"
                        ...
                        {/* Permanent delete — owner-only (server
                            enforces it too). The only way a
                            deactivated member's seat is freed. */}
                        {isOwner && (
                          <Button
                            variant="outline"
                            size="sm"
                      ...
                      <>
                        {/* Role display / editor. Inline Select is
                            admin+ only AND not allowed on the owner row
                            (owner changes go through transfer, which
                            lands later). */}
                        {canManageMembers && !isOwnerRow && !isSelf ? (
                          <Select
                            value={member.role}
                            onValueChange={(v) =>
                              // Base UI Select can emit null on clear.
                              // We don't expose a clear affordance, so
                              // the guard is defensive — but the typed
                        ...
                        {/* Remove (= deactivate). Admin+ only; never on
                            the owner row; never on yourself.
                            Pre-polish styling was neutral-default +
                            red-on-hover — the destructive intent was
                            invisible until the user moused over. Now
                            red is the default state with a darker
                            shade on hover so the affordance reads
                            at-a-glance. */}
                        {canManageMembers && !isOwnerRow && !isSelf && (
                          <Button
                            variant="outline"
      ...
      {/* Permanent delete — owner-only; irreversible. Frees the seat
          taken by a deactivated member. */}
      <Dialog
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {t.rich('deleteDialogDesc', {
                name: deletingMember?.full_name || t('unnamed'),
                bold: (chunks: React.ReactNode) => <strong>{chunks}</strong>,
              })}
            </DialogDescription>
          </DialogHeader>
