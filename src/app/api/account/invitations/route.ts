...
export async function POST(request: Request) {
  try {
    ...
    // Refuse to create an invite for a team with no free seat. The DB
    // trigger (migration 047) enforces the same rule as defense-in-
    // depth; this check surfaces a clean 400 and skips the INSERT.
    const [
      { data: usage, error: usageErr },
      { data: seatLimit, error: seatLimitErr },
    ] = await Promise.all([
      ctx.supabase.rpc('account_member_usage', {
        p_account_id: ctx.accountId,
      }),
      ctx.supabase.rpc('account_member_limit', {
        p_account_id: ctx.accountId,
      }),
    ]);

    if (usageErr || seatLimitErr) {
      console.error(
        '[POST /api/account/invitations] seat check error:',
        usageErr ?? seatLimitErr
      );
      return NextResponse.json(
    ...
    if ((usage ?? 0) >= (seatLimit ?? 0)) {
      return NextResponse.json(
        {
          error: `This team has reached its member limit (${seatLimit ?? 0} seats). An owner must permanently delete a member to free a seat.`,
          code: 'seat_full',
        },
        { status: 400 }
      );
    ...
    if (error || !data) {
      // The seat-cap trigger (047) can reject the insert if this team's
      // seats filled between the check above and the INSERT.
      if (error?.code === '22023') {
        return NextResponse.json(
          {
            error:
              'This team has reached its member limit. An owner must permanently delete a member to free a seat.',
            code: 'seat_full',
          },
          { status: 400 }
        );
